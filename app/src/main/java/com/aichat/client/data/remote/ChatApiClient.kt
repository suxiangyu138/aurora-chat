...
class ChatApiClient {
        ...
        return EventSources.createFactory(clientWithTimeout(config))
            .newEventSource(request, object : EventSourceListener() {
                ...
                override fun onEvent(eventSource: EventSource, id: String?, type: String?, data: String) {
                    val delta = parseStreamData(data)
                ...
                override fun onFailure(eventSource: EventSource, t: Throwable?, response: Response?) {
                    ...
                    mainHandler.post {
                        ...
                        if (fullText.isEmpty()) onError(message) else onComplete(fullText.toString())
                    }
                }
            })
    }

    /** 解析 SSE 事件数据:兼容 [DONE]、多行 data、各厂商差异 */
    private fun parseStreamData(data: String): String? {
        ...
        for (line in data.lines()) {
            ...
            extractContent(trimmed)?.let { sb.append(it) }
        }
    ...
    /** 非流式单次问答 */
    suspend fun chatOnce(config: ModelConfig, messages: List<ChatMessage>): Result<String> =
    ...
    /** 流式增量:choices[0].delta.content(兼容 choices[0].message.content) */
    private fun extractContent(json: String): String? = runCatching {
